...
if (not path.exists(CONFIG_ABSPATH)):
    print(neutral_msg('No se encontró un fichero config.yml válido en el directorio del script, se buscará en el cwd actual'))
    CONFIG_ABSPATH = Path(getcwd()).joinpath(CONFIG_FILENAME)
...
try:
    ...
    print(neutral_msg('No se encontró un archivo config.yml válido en el cwd, usando valores por defecto...'))
